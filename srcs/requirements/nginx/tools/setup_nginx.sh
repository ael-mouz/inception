...
openssl req -x509 -new -key "${NGINX_CERTS_PATH}private-key.key" -out "${NGINX_CERTS_PATH}certificate.crt" -days 365 -subj "/C=MA/ST=State/L=City/O=Organization/OU=Organizational Unit/CN=localhost" 2>/dev/null
